...
class AvgSessions extends React.Component {
  ...
  _getResources() {
    ...
    .then(({status, message, data}) => {
        if (status) {
            this.setState({ companies: data.users, loading: false })
        }
    })
}    
  ...
  async _getReports() {
    ...
    let c = new WebClient();
    await c.post("report-5.json", {start: this.state.start, end: this.state.end, company_id: this.state.selected_company?.value ?? 0})
    .then(({status, message, data}) => {
        console.log(data)
      if (status) {
    ...
    return (
        ...
        <Col md="12">
            ...
            <Row>
                ...
                <Col sm="6">
                      <FormGroup>
                      <CLabel required for="EmailVertical">Kullanıcı</CLabel>
                      <Select
